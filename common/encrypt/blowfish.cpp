...
void Blowfish::setKey(const unsigned char* key, int byte_length) {
    ...
    memcpy(m_pary_, initial_pary, sizeof(initial_pary));
    memcpy(m_sbox_, initial_sbox, sizeof(initial_sbox));

    static const int m_pary_length = sizeof(m_pary_) / sizeof(uint32_t);
    static const int m_sbox_length = sizeof(m_sbox_) / sizeof(uint32_t);

    {
        ...
        for (int i = 0; i < m_pary_length; ++i) {
            uint32_t key_uint32 = key_buffer[i % buffer_length];
            m_pary_[i] ^= key_uint32;
        }
    ...
    for (int i = 0; i < (m_pary_length / 2); ++i) {
    ...
    for (int i = 0; i < (m_sbox_length / 2); ++i) {
        encryptBlock(&left, &right);

        reinterpret_cast<uint32_t*>(m_sbox_)[i * 2] = left;
        reinterpret_cast<uint32_t*>(m_sbox_)[i * 2 + 1] = right;
    }
}

void Blowfish::encrypt(unsigned char* dst, const unsigned char* src, int byte_length) {
    if (dst != src) {
...
void Blowfish::decrypt(unsigned char* dst, const unsigned char* src, int byte_length) {
    if (dst != src) {
        memcpy(dst, src, byte_length);
    }
...
uint32_t Blowfish::feistel(uint32_t value) {
    Converter32 converter;
    ...
    return ((m_sbox_[0][a] + m_sbox_[1][b]) ^ m_sbox_[2][c]) + m_sbox_[3][d];
}
